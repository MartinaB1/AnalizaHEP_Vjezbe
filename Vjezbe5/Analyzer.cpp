...
{
  TH1F* Histo_pT1 = new TH1F("pT1","pT1",50,0,300);//objekt klase(ime,titl,koje su oznake na osima, odakle, dokle crtamo)  
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     if (ientry < 0) break;
     nb = fChain->GetEntry(jentry);   nbytes += nb;
     Histo_pT1->Fill(sqrt(pow(DecayParticle1_px,2)+pow(DecayParticle1_py,2)));//punim sa transverzalnom kolicinom gibanja
     Histo_pT2->Fill(sqrt(pow(DecayParticle2_px,2)+pow(DecayParticle2_py,2)));
     P1.SetPxPyPzE(DecayParticle1_px,DecayParticle1_py,DecayParticle1_pz,DecayParticle1_E);//definira se cetverovektor
